...
int sumucn(int n);
int sumucld(int ld);
void check(int n, int dc, int dg, int ld);
void xuat(int bc, int bg, float n);
...
int main() {
	...
	xuat(bc, bg, nd);						//Xuat ket qua vao output.out
}

void check(int n, int dc, int dg, int ld) {							//Kiem tra du lieu dau vao

    if (n <= 0 || n > 1000 || dc <= 0 || dg <= 0 || (ld < 1 || ld > 300)) {
        int bc = -1;
        int bg = -1;
       // float nd = n;
        xuat(bc, bg, n);
...
void xuat(int bc, int bg, float nd) {				//Ham xuat ket qua vao output
	FILE *f;
...
void update_I(int n, int dc, int dg, int ld, char *w) {				//Update input
